...
class GradeSpider:
    ...
    def hello_grade(self):
        ...
        self.years = get_select(selector.xpath('//*[@id="ddlXN"]/option[1]/following-sibling::option'))
        self.semester = get_select(selector.xpath('//*[@id="ddlXQ"]/option[1]/following-sibling::option'))[:-1]
    ...
    def query_by_xq(self,courses):
        result = []
        if courses is not None:
            print("当前可查询的学期有:")
            for i in range(len(self.semester)):
                print(str(i) + " . " + self.semester[i])
            xq = self.semester[int(input("请根据数字选择学期: "))]
            for course in courses:
                if xq == course.semester:
                    result.append(course)
        return result

    def query_by_xn(self,courses):
        print("当前可查询的学年有:")
        for i in range(len(self.years)):
            print(str(i) + " . " + self.years[i])
        xn = self.years[int(input("请根据数字先选择学年: "))]
        result = []
        for course in courses:
    ...
    def query_by_name(self, course_name, courses):
        result = []
        for course in courses:
    ...
    def get_final_grade(self):
        while True:
            ...
            courses = get_grade(content)
            if cmd == 0:
                courses = self.query_by_xn(courses)
                courses = self.query_by_xq(courses)
                break
            elif cmd == 1:
                courses = self.query_by_xn(courses)
                break
                ...
                while True:
                    ...
                    if len(courses) != 0:
                        break
                    print("没有查询到包含关键字: \'"+ course_name +"\' 的课程成绩")
